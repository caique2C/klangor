    ...
    MethodChannel.MethodCallHandler
{
    ...
    // Vendored+patched copy of flutter_pcm_sound 3.3.3 (see
    // packages/flutter_pcm_sound/PATCHES.md). Upstream hardcodes this at 200
    // frames (~4.2ms @ 48kHz), so playbackThreadLoop() below - which does a
    // full synchronized scan of the *entire* remaining queue on every single
    // iteration to compute remaining-frame stats - has to iterate roughly
    // 240x/second just to keep up with real-time playback, at
    // Thread.MAX_PRIORITY. Confirmed via `adb shell top -H` that this alone
    // pins the native playback thread near 100% CPU continuously during
    // perfectly healthy playback (no underruns/overflows), which is enough
    // to visibly degrade UI scroll responsiveness system-wide since it can
    // preempt normal-priority work. Raised to 4800 frames (~100ms @ 48kHz,
    // matching this app's own feed-threshold scale in pcm_audio_player.dart)
    // to cut loop iterations ~24x. This has no effect on pause latency -
    // pause() discards whatever's queued natively via release() regardless
    // of how finely it was internally split.
    private static final int MAX_FRAMES_PER_BUFFER = 4800;
